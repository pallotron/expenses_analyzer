...
class TestValidation(unittest.TestCase):
    ...
    def test_missing_date_column_fails(self) -> None:
        ...
        df = pd.DataFrame({"Merchant": ["Store A"], "Amount": [10.50]})
    ...
    def test_missing_merchant_column_fails(self) -> None:
        ...
        df = pd.DataFrame({"Date": [datetime(2025, 1, 1)], "Amount": [10.50]})
    ...
    def test_missing_amount_column_fails(self) -> None:
        ...
        df = pd.DataFrame({"Date": [datetime(2025, 1, 1)], "Merchant": ["Store A"]})
    ...
    def test_dates_too_new_fail(self) -> None:
        ...
        df = pd.DataFrame(
            {"Date": [future_date], "Merchant": ["Store A"], "Amount": [10.50]}
        )
    ...
    def test_multiple_errors_reported(self) -> None:
        ...
        assert "merchant" in errors_str
        assert (
            "amount" in errors_str
            or "negative" in errors_str
            or "numeric" in errors_str
        )

    def test_zero_amounts_allowed_with_warning(self) -> None:
        """Test that zero amounts are allowed but warned about."""
        df = pd.DataFrame(
            {"Date": [datetime(2025, 1, 1)], "Merchant": ["Store A"], "Amount": [0.00]}
        )
    ...
    def test_validation_error_has_errors_list(self) -> None:
        ...
        df = pd.DataFrame(
            {"Date": ["invalid"], "Merchant": [""], "Amount": ["not a number"]}
        )
